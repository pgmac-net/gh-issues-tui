...
Backend-specific features are **capabilities**: trait methods with safe defaults (`Err(ProviderError::Unsupported)`) paired with a `supports_*` probe the UI checks before offering the affordance. There are two today: the linked-PR summary popup (`supports_pr_summary` / `pull_request` — GitHub only) and moving an issue to another repo (`supports_move` / `move_issue` — GitHub and Linear; Jira falls back to the default). A provider that opts out gets a status-bar message instead of a doomed fetch. Domain types stay in `provider/types.rs` even when only one backend can fetch them today (e.g. `PrSummary`): the data is neutral, the fetch is a capability.
...
**Capabilities.** `supports_pr_summary` stays `false` (no GitHub PR links in Linear), so the `P` keybind degrades to a status message via the #63 capability gate. `supports_move` is `true`: `move_issue` resolves the target team key to an id (`teams(filter: {key: {eq:}})`, the same lookup `real_repo_labels` uses) and calls `issueUpdate(teamId:)` — Linear remaps the issue's workflow state and label set to the target team server-side, so the provider makes no attempt to reconcile them itself (#135). Milestones and issue types have no Linear equivalent and stay empty; `projects` maps to Linear projects. Comment count is not fetched in the bulk list query (it appears when the detail pane loads the thread).
...
**The fetch is two-phase, and the split is a cost decision, not a structural one** (#107). GitHub bills GraphQL against a points budget where `cost = 1 + n_nested_connections × (reposFirst × issuesFirst) / 100` — a connection nested inside `issues` is charged per issue *slot requested*, regardless of its own page size or how much data comes back. Carrying `assignees` and `labels` inline cost 101 points a page (202 a refresh, 2,424/hour at the default interval). So the bulk query (`ORG_ISSUES_QUERY` / `REPO_ISSUES_QUERY`, via the `issue_fields!` macro) requests **scalars only** — cost 1 for the whole org — and `Client::hydrate_issues` fills in labels and assignees with `nodes(ids:)` batches of 100, at 2 points a batch. Measured end-to-end: **4 points, down from 202**. `issue_fields_has_no_nested_connections` fails the build if a connection reappears there. Full measurements and the rejected alternatives: `docs/graphql-api-cost.md`.

Hydration is atomic — `org_issues` returns fully populated issues — because `provider::priority` derives priority from labels and the label filters read them; a progressive render would sort the list, then reshuffle it when hydration landed.

`reposFirst` is 100 (GitHub's maximum) precisely because cost no longer scales with it: the largest page is also the cheapest, needing fewer requests for the same points.
...
- **Consistency**: mutations trigger a full refetch on completion rather than optimistic patching — simpler, and correct by construction. When the detail pane is open, the same completion also refetches the open issue's comment thread (invalidating its cache entry first, since the mutation may have been that comment), so a just-added comment appears without navigating away and back.
- **Comment threads are cached per issue** (`App::comment_cache`, keyed by issue id). `App::load_comments` returns an issue id only when a request is genuinely needed: a thread already fetched this cycle, or an issue whose `comment_count` is zero, resolves with no API call — navigating the list with the detail pane open used to cost one request per row. Both cases settle `detail.comments` to a loaded (possibly empty) thread so the pane never waits on a response that will never arrive. The cache is cleared wholesale by `set_data` and `switch_org`.
...
| move to another repo | `repository(owner:,name:){id}` lookup, then `transferIssue(repositoryId:, createLabelsIfMissing: true)` |
...
`m` (#135) moves the selected issue: `Mode::MovePicker` lists every other repo already loaded for the org (`App::move_targets`), then commits into `Mode::ConfirmMove`, a confirmation popup naming the destination and the transfer's side effects (new issue number, mentioned users notified). Full detail: `docs/move-issue.md`.

Requiring a same-owner target isn't a corner this implementation cut — GitHub's `transferIssue` genuinely rejects cross-owner transfers, so the picker's targets (every repo the org fetch already returned) are exactly its legal target set; there is nothing to fetch for the picker to open.

The confirmed target doesn't ride inside `Mode` the way `HarnessConfirm::Kill(SessionId)` does, because a `String` payload would cost `Mode` its `Copy` bound. It lives in `App::pending_move: Option<PendingMove>` instead, captured at picker-commit time — the same "resolve now, recheck at commit" shape as `PickerState::priority_issue`/`label_issue` — so a refetch or selection change while the confirm popup is open can't retarget the mutation onto a different issue; `confirm_move` checks the captured issue id against the current selection before dispatching.
