...
- **Wrapped-height measurement** uses ratatui's `Paragraph::line_count` (enabled via the `unstable-rendered-line-info` feature) so the key handler's scroll clamps match the drawn layout exactly — the old source-line-count / lower-bound-offset caveat is gone. `body_lines` and `comment_card_lines` in `ui.rs` build the region content once; the renderer draws them (real theme) and `body_content_height` / `comment_height` / `comment_offset` measure them (default theme — styling doesn't affect wrapping). The key handler recovers the live region widths/heights from the terminal size via `detail_metrics` (mirrors `ui::draw`'s split arithmetic).
