...
- **Inferred priority ranks are read-only, and opt-in twice (#156).** `Label::rank` (from `typesafe/`) feeds sort and title colour — never `priority_set_options` or `priority_label_set`, so a wrong judgement can mis-sort but never change a label on a backend. It runs only with `infer_priority_ranks = true` in config **and** `TYPESAFE_API_KEY` in the env; only label names are sent. `priority:*` always wins over an inferred rank. If the question wording, levels or model change: bump `typesafe::PROMPT_VERSION` and re-record with `cargo test calibrate_against_live_api -- --ignored --nocapture` (needs the key) — offline tests over `typesafe/calibration.json` fail CI otherwise, and read the table before touching `MIN_CONFIDENCE`. Full detail: `docs/priority-rank-inference.md`, `docs/adr/0002-*`.
