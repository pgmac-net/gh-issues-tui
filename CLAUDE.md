...
- Comments render as **cards** (`draw_detail`): a header rule embedding author · timestamp, the body, and a bottom rule, so each comment's bounds are obvious. The pane has a **card cursor** (`detail_card`: 0 = issue body, 1..=N = comments) that `j/k` move while `Focus::Detail` (they navigate the list otherwise); `move_detail_card` scrolls the selected card's top into view via `detail_card_offset`, whose line arithmetic mirrors `draw_detail` (shared `comment_card_lines` helper keeps the two in sync — wrapping isn't modelled, so the offset is a lower bound). `e` edits the highlighted card.
- **Inline editing** reuses the one `Mode::CommentEditor` widget (multi-line editor + Save/Cancel) for three targets via `EditorTarget` on `App`: `NewComment` (`c`), `EditComment { comment_id }` and `EditBody` (`e` on a comment / the body card). `start_edit_selected_card` prefills the editor with the current text (`BodyEditor::from_text`); `submit_comment` branches on the target to `add_comment` / `update_comment` / `update_body`, then the usual `MutationDone` refetch. An empty submission is discarded except for `EditBody` (clearing a description is valid).
