...
- **Harness sessions (#23)** are coding agents on real PTYs, one per issue, drawn full-frame. The split is the point: `App.harness` holds pure metadata only, while the PTYs live in a `HarnessRegistry` owned by the event loop and passed to key handlers as `HarnessCtx` — that is what keeps `app/` I/O-free and lets every lifecycle transition be tested without spawning anything. Session output never crosses the event channel (the reader thread parses into its own `vt100::Parser` and sends a payload-free `HarnessDirty(id)`, which the loop uses only to skip redraws for offscreen sessions). Session ids are monotonic and never reused, so a late event cannot address a session that took its slot. One session per issue is enforced in `HarnessCtx::launch`, not in `launch_action` — the picker and `F12 n` bypass the latter. Harness commands are argv arrays, never shell strings: issue text is attacker-controlled. Full detail: `docs/harness-sessions.md`.
