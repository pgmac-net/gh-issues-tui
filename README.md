...
`infer_priority_ranks` makes priority sort, title colouring, the priority filter picker and the `p` (set priority) picker work on repos that label priority `P0`/`P1`, `sev1`, `blocker` and the like rather than `priority:high`. It asks TypeSafe's System One model to rate each unfamiliar label name, and needs `TYPESAFE_API_KEY` in the environment as well — either one missing leaves it off. Only label *names* are sent, never titles, bodies or the org name.
...
`send_issue_text` lets the app send issue **text** — titles, bodies and comment text — to TypeSafe, for the ticket-readiness badge in the detail pane and for **semantic `/` search**, which adds issues judged to be *about* the query to the usual substring hits a few seconds later ([`docs/semantic-search.md`](docs/semantic-search.md)). It needs `TYPESAFE_API_KEY` too. This is a separate and much larger disclosure than `infer_priority_ranks`, which sends only label names: a private issue thread is often the most sensitive text in a repo. The two flags are independent in both directions, and neither ever sends the org or repo name. The badge is advisory — it cannot affect, delay or block a harness launch. Details: [`docs/ticket-readiness.md`](docs/ticket-readiness.md) and [`docs/adr/0004-*`](docs/adr/0004-issue-text-may-be-sent-behind-a-second-consent.md).
